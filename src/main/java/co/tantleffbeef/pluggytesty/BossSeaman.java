...
public class BossSeaman implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        ...
        BukkitRunnable runnable = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (attack == 1 && seaman.hasLineOfSight(target) && loc.distance(target.getLocation()) > 4) { // ride trident towards target
                    seaman.setVelocity(dir.multiply(3));
                } else if (attack == 2) { // 8 tridents at 45 degrees
                ...
                } else if (attack == 3 && loc.distance(target.getLocation()) < 5) { // charge up and summon lightning if not interrupted.
                ...
                } else if (attack == 4) { // bubble pulse
                    pulse(seaman.getLocation(), seaman);
                } else if (attack == 5) { // regenerate
                    seaman.addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 30, 3));
                }
    ...
    private void pulse(Location location, Drowned seaman) {
        BukkitRunnable runnable = new BukkitRunnable() {
            int runs = 0;
            Location l = location;
            final BlockData blockParticle = Material.BUBBLE_COLUMN.createBlockData();
            ...
            public void run() {
                ...
                w.playSound(l, Sound.ENTITY_PLAYER_SPLASH, 8, 0.1f);
